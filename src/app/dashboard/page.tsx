...
import { Title, Text, SimpleGrid, Card, Badge, Table, TableThead, TableTbody, TableTr, TableTh, TableTd, Alert, Group, Anchor } from "@mantine/core";
...
export default async function DashboardPage() {
  ...
  const rows = userProfile.transactions.map((tx) => (
    <TableTr key={tx.id}>
      ...
      <TableTd>
        <Badge 
          color={tx.status === 'PAID' ? 'green' : tx.status === 'PENDING' ? 'yellow' : 'red'}
          variant="light"
        >
          {tx.status}
        </Badge>
    ...
    <FadeIn>
      ...
      {!userProfile.linearId && (
        <Alert color="yellow" title="Linear Account Not Linked" mb="xl">
          We couldn't automatically link your Linear account. Please ensure your Clerk account email ({user?.primaryEmailAddress?.emailAddress || 'Not set'}) matches your Linear workspace email, or connect Linear via your profile settings.
        </Alert>
      )}
